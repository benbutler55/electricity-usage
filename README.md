...
- **Discharge-power warning**: `assessOutputPower` flags a battery whose `output_kw` is below the home's busiest hourly-average demand (`peakDemandKw`). The conservatism is a single knob, `PEAK_POWER_SAFETY_FACTOR` in `src/lib/agileZones.ts`.

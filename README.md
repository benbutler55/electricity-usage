...
- **Battery savings formula**: `saving = effectiveKwh × (avgPeakRate − avgChargeRate ÷ efficiency)`. Effective kWh is capped two ways to avoid over-stating savings: by your typical peak-hour consumption (from the heatmap), and by the battery's **discharge power** — each slot can release at most `output_kw × slot-hours`, so a low-output battery can't dump its whole capacity into a short peak window. Charge energy is bounded separately by `charge_rate_kw`.
